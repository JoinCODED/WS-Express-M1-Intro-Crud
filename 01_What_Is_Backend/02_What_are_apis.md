`API` is the acronym for `Application Programming Interface`, which is a software that allows two applications to talk to each other even if they are using different programming languages.
